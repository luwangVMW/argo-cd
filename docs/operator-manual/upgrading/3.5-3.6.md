...
`ARGOCD_K8S_SERVER_SIDE_TIMEOUT` now applies to each attempt of a non-long-running Kubernetes API
request. Previously, the value was assigned to the REST client's global timeout, which also
terminated watch, streaming log, exec, attach, port-forward, and proxy connections when the
configured duration elapsed.

> [!NOTE]
> When `ARGOCD_K8SCLIENT_RETRY_MAX` is greater than zero, each retry attempt receives a new timeout.
> The maximum duration of the complete operation can therefore exceed
> `ARGOCD_K8S_SERVER_SIDE_TIMEOUT` because it includes every attempt and the retry backoff.
