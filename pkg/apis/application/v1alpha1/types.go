...
func SetK8SConfigDefaults(config *rest.Config) error {
	...
	// rest.Config.Timeout is a client-side HTTP timeout that covers the entire
	// request lifetime, including watches and streaming pod logs. Keep it disabled
	// so long-running connections are not terminated; the transport wrapper sends
	// the timeout to the API server instead.
	config.Timeout = 0
	config.Transport = tr
